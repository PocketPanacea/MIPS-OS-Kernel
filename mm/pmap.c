...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
	...
	for( i = 0; i < size ; i += BY2PG ){
		*(boot_pgdir_walk(pgdir,va+i,1)) = PTE_ADDR(pa+i)|perm|PTE_V;
	}
...
page_init(void)
{
	...
	pages[0].pp_ref = 1;
	for( i = 0; i < npage; i++ ){
		if(i < ROUND(PADDR(freemem),BY2PG)/BY2PG)
			pages[i].pp_ref = 1;
		else{
			pages[i].pp_ref = 0;
...
pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte)
{
	...
	if( *targetPde == 0 ){
		if( create == 0 ){
			...
			*targetPde = page2pa(pageTablePage)|PTE_R|PTE_V;
