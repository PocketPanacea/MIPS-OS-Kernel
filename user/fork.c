...
pgfault(u_int va)
{
    ...
    if (syscall_mem_alloc(0, PFTEMP, PTE_R|PTE_V) < 0)
        user_panic("syscall_mem_alloc failed!");

    user_bcopy((void*)va, PFTEMP, BY2PG);

    if (syscall_mem_map(0, PFTEMP, 0, va, PTE_R|PTE_V) < 0)
...
duppage(u_int envid, u_int pn)
{
    ...
    {
        writef("");	
        //perm = PTE_V|PTE_COW | PTE_LIBRARY;
        if(perm & PTE_LIBRARY) {
            perm = perm | PTE_V | PTE_R | PTE_LIBRARY;
        }
        else{
